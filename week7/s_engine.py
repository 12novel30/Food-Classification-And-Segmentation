...
import torch
import torchvision
from s_coco_eval import CocoEvaluator
...
def train_one_epoch(epoch, model, data_loader, optimizer, device, 
    ...
    model.train()
    # logger
    metric_logger = s_utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', s_utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    ...
    for idx, (images, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        optimizer.step()
        # sum loss
        loss_dict_reduced = s_utils.reduce_dict(loss_dict)
